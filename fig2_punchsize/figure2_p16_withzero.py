...
transf = mtransforms.Affine2D().skew_deg(-26.5651,0).translate(3.4+.6, 0)
trans_data = transf + ax1.transData
im1a.set_transform(trans_data)
sro_z0a = sro_z0[:-1,:-1]
#combo = np.roll(sro_z0a,[0,8], axis=(0,1))
combo = np.roll(sro_z0a,[16,8], axis=(0,1)) + np.roll(sro_z0a,[8,16], axis=(0,1))
combo += 0.25*(np.roll(sro_z0a,[16,0], axis=(0,1)) + np.roll(sro_z0a,[8,8], axis=(0,1))+ np.roll(sro_z0a,[0,16], axis=(0,1)))
######### difference
...
transf = mtransforms.Affine2D().skew_deg(-26.5651,0).translate(-1.7, dy)
trans_data = transf + ax1.transData
...
transf = mtransforms.Affine2D().skew_deg(-26.5651,0).translate(3.4+.6, dy)
trans_data = transf + ax1.transData
im3a.set_transform(trans_data)

sro_z0a = sro_z0[:-1,:-1]
combo = np.roll(sro_z0a,[0,16], axis=(0,1)) #main monoclinic
combo += 0.22*(np.roll(sro_z0a,[16,0], axis=(0,1))+ np.roll(sro_z0a,[8,8], axis=(0,1))) #secondary monoclinic
combo += 0.17*(np.roll(sro_z0a,[8,16], axis=(0,1))+ np.roll(sro_z0a,[16,8], axis=(0,1))) #rhombohedral
combo += 0.1*np.roll(sro_z0a,[0,0], axis=(0,1)) 
combo1 = np.zeros((25,25))
combo1[:-1,:-1] += combo
combo1[:-1,-1] += combo[:,0]
combo1[-1,:-1] += combo[0,:]
...
transf = mtransforms.Affine2D().skew_deg(-26.5651,0).translate(0, dy)
trans_data = transf + ax1.transData
...
y0 = 4.57916+0.1
d = 7
ax1.set_xlim(-0.95,-0.95+d*1.08)
ax1.set_ylim(2.1,2.1+d*3**-.5)
...
drawcell(x0,y0,ax1)
drawcell(x0+2*1.7+.6,y0,ax1)
drawcell(x0+2*1.7+.6,y0-dy,ax1)
drawcell(x0,y0-dy,ax1)
drawcell(x0-1.7,y0,ax1)
drawcell(x0-1.7,y0-dy,ax1)
...
def drawcell1(x,y,ax):
    ...
    return
drawcell1(x0+1.7,y0-dy,ax1)
drawcell1(x0+1.7,y0,ax1)

x1 = 1.18
ax1.text(x1-1.7, 5.8, '300 K, z=1/3')
ax1.text(x1-1.7, 5.5-1.4, '30 K , z=1/3')
ax1.text(x1-0.4, 5.8, '300 K, weighted sum z=0')
ax1.text(x1-0.4, 5.5-1.4, '30 K, weighted sum z=0')
ax1.text(x1+1.6, 5.8, '300 K stacking')
ax1.text(x1+1.6, 5.5-1.4, '30 K stacking')
ax1.text(x1+4, 5.8, '300 K, z=0')
ax1.text(x1+4, 5.5-1.4, '30 K, z=0')

ax1.text(x0+.47,4.46, 'a')
ax1.text(x0+.47+1.7,4.46, 'a')
ax1.text(x0+.47+1.7,4.26-1.4, 'a')
ax1.text(x0+.47,4.26-1.4, 'a')
ax1.text(x0+.47+1.7*2+.6,4.46, 'a')
ax1.text(x0+.47+1.7*2+.6,4.26-1.4, 'a')
ax1.text(x0+.47-1.7,4.46, 'a')
ax1.text(x0+.47-1.7,4.26-1.4, 'a')
...
ax1.text(x0+1.3,y0-1.25, 'b')
ax1.text(x0-0.4,y0-1.25, 'b')
ax1.text(x0+1.3+1.7+.6,y0+0.45, 'b')
ax1.text(x0+1.3+1.7+.6,y0-1.25, 'b')
ax1.text(x0-2.1,y0+0.45, 'b')
ax1.text(x0-2.1,y0-1.25, 'b')

da = 0.333*np.array([1,0])
db = 0.333*np.array([-0.5, 1])
vec = da*2
vec *= 0.85
ax1.arrow(x0+1.7, y0, vec[0], vec[1], color='g', head_width=0.08, zorder=20)
vec = db*2
vec *= 0.85
ax1.arrow(x0+1.7, y0, vec[0], vec[1], color='g', head_width=0.08, zorder=20, linestyle='dotted')
vec = db+da
vec *= 0.75
ax1.arrow(x0+1.7, y0, vec[0], vec[1], color='g', head_width=0.08, zorder=20, linestyle='dotted')
...
vec = da*2+db
vec *= 0.9
ax1.arrow(x0+1.7, y0, vec[0], vec[1], color='k', head_width=0.05)
vec = da+2*db
vec *= 0.95
ax1.arrow(x0+1.7, y0, vec[0], vec[1], color='k', head_width=0.05)
ax1.plot(x0+1.7, y0, marker='o', color='r', ms=3)
ax1.plot(x0+1.7, y0, marker='o', mec='r', mfc='None',ms=8)
...
ax1.arrow(p1, p2-1.7, dxl, 0,  color='g', head_width=0.03)
ax1.arrow(p1, p2-.12-1.7, dxl, 0,  color='k', head_width=0.03)

ax1.text(p1+0.18, p2-.03-1.7, '73%, R-3')
ax1.text(p1+0.18, p2-0.12-.03-1.7, '27%, C2/m')

vec = da*2 +db
vec *= 0.85
ax1.arrow(x0+1.7, y0-1.7, vec[0], vec[1], color='g', head_width=0.08)
vec = db*2 +da
vec *= 0.85
ax1.arrow(x0+1.7, y0-1.7, vec[0], vec[1], color='g', head_width=0.08)
...
vec = 2*da
vec *= 0.9
ax1.arrow(x0+1.7, y0-1.7, vec[0], vec[1], color='k', head_width=0.05)
vec = 2*db
vec *= 0.95
ax1.arrow(x0+1.7, y0-1.7, vec[0], vec[1], color='k', head_width=0.05)
...
fig.text( 0.227, 0.96, '(c)')
fig.text(0.452, 0.96, '(e)')
fig.text(0.757, 0.96, '(g)')


fig.text( 0.005, 0.54, '(b)')
fig.text( 0.227, 0.54, '(d)')
fig.text(0.452, 0.54, '(f)')
fig.text(0.757, 0.54, '(h)')
