...
z1 = 0
dat = a['processed/fft_interp_p12'][z1-0.02:z1+0.02, y:y+1, x:x+1].sum(axis=0)
sro_z0 = dat.data.nxvalue
combo = np.roll(sro_z0,[16,8], axis=(0,1)) + np.roll(sro_z0,[8,16], axis=(0,1))
combo += 0.18*(np.roll(sro_z0,[16,0], axis=(0,1)) + np.roll(sro_z0,[8,8], axis=(0,1))+ np.roll(sro_z0,[0,16], axis=(0,1)))
######### difference
...
combo += 0.16*(np.roll(sro_z0,[16,0], axis=(0,1))+ np.roll(sro_z0,[8,8], axis=(0,1))) #secondary monoclinic
combo += 0.12*(np.roll(sro_z0,[8,16], axis=(0,1))+ np.roll(sro_z0,[16,8], axis=(0,1))) #rhombohedral
combo += 0.07*np.roll(sro_z0,[0,0], axis=(0,1)) 
#######difference
#combo = sro_dat/np.amax(sro_dat) - combo/np.amax(combo)
...
ax1.text(x1+1.45, 5.7, '300 K, linear comb. z=0')
ax1.text(x1+1.45, 5.5-1.4, '30 K, linear comb. z=0')
...
ax1.text(p1+0.18, p2-.03, '59% C2/m')
ax1.text(p1+0.18, p2-0.12-.03, '19% C2/m')
ax1.text(p1+0.18, p2-0.24-.03, '14%, R-3')
ax1.text(p1+0.18, p2-0.36-.03, '7%, no shift')
...
fig.text( 0.1, 0.9, '(a)')
fig.text(0.36, 0.9, '(b)')
fig.text(0.625, 0.9, '(c)')

fig.text( 0.1, 0.44, '(d)')
fig.text(0.36, 0.44, '(e)')
