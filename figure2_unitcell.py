...
# OUT OF DATE - use the p16 file in the punchsize subfolder
...
combo += 0.18*(np.roll(sro_z0,[16,0], axis=(0,1))+ np.roll(sro_z0,[8,8], axis=(0,1))) #secondary monoclinic
combo += 0.12*(np.roll(sro_z0,[8,16], axis=(0,1))+ np.roll(sro_z0,[16,8], axis=(0,1))) #rhombohedral
combo += 0.12*np.roll(sro_z0,[0,0], axis=(0,1))#######difference
#combo = sro_dat/np.amax(sro_dat) - combo/np.amax(combo)
